...
class AdminTradesetController extends AdminBaseController
{
    protected $name = 'member_tradeset';
    protected $flag = '交易设置';
    private $db;
    ...
    public function delete()
    {
        ...
        if (isset($param['id'])) {
            $id     = $this->request->param('id', 0, 'intval');
            $find   = $this->db->field('member_id')->where('id', $id)->find();
            $result = $this->db->where('id', $id)->delete();
            if ($result) {
                c2c_admin_log($this->name .':'. $id, '删除'. $this->flag .'：' . $find['member_id']);
    ...
    public function opp($valid = 'add', $data = [])
    {
        ...
        $result            = $this->validate($data, 'Tradeset.'.$valid);
        if ($result !== true) {
            $this->error($result);
        }
