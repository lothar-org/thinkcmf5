...
class PortalPostModel extends Model
{
    ...
    public function addTags($keywords, $articleId)
    {
        ...
        $mq = Db::name('portal_tag_post');
        $tagIds = [];
        $data = [];

        if (!empty($keywords)) {
            $oldTagIds = $mq->where('post_id', $articleId)->column('tag_id');
            ...
            // 清空tag关系。 只有旧数据时，说明表单中关键词为空且之前有数据，这是编辑时产生的
            if (empty($tagIds) && !empty($oldTagIds)) {
                $mq->where('post_id', $articleId)->delete();
            }

            // 删除旧tag关系。 没有条件，但排除上面的情况
            // 有旧的和新的。  无旧的有新的、全无。  有旧的无新的属于上面的。
            // 即等同于：
            // if (!empty($tagIds) && !empty($oldTagIds)) {
            // }
            ...
            if (!empty($shouldDeleteTagIds)) {
                $mq->where(['post_id' => $articleId, 'tag_id' => ['in', $shouldDeleteTagIds]])->delete();
            }
            // 加新的
            if (!empty($data)) {
                $mq->insertAll($data);
